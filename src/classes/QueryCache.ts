...
export class RedisQueryCache implements QueryCacheProvider<Track> {
    ...
    public async addData(data: SearchResult): Promise<void> {
        ...
        const serialized = JSON.stringify(
            data.playlist ? serialize(data.playlist) : data.tracks.map((track) => serialize(track))
        );

        await this.redis.setex(key, this.EXPIRY_TIMEOUT, serialized);
    }

    public async getData(): Promise<DiscordPlayerQueryResultCache<Track<unknown>>[]> {
        ...
        const parsed = serialized.filter(Boolean).map((item) => deserialize(player, JSON.parse(item!))) as Track[];
    ...
    public async resolve(context: QueryCacheResolverContext): Promise<SearchResult> {
        ...
        try {
            ...
            const raw = JSON.parse(serialized) as SerializedTrack[] | SerializedPlaylist;
