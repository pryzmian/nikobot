...
export default class PauseButton extends BaseComponent {
    public constructor() {
        super({
            name: 'pause-button'
        });
    ...
    public async execute(params: BaseComponentParams): Promise<void> {
        ...
        if (!queue) {
            await this.sendEmbedResponse(interaction, 'There is no queue playing in this server!', '#2B2D31');
            return;
        }
        ...
        try {
            ...
            await this.sendEmbedResponse(
                interaction,
                `${queue.node.isPaused() ? 'Paused' : 'Resumed'} the current track!`,
                '#2B2D31'
            );
        } catch (error) {
            console.error('(button-pause) An error occurred while pausing the player:', error);
            return;
        }
    }
